 ...
 */
final class ASMS_GitHub_Updater {
	private const OWNER               = 'cchatterton';
	private const REPO                = 'as-ms-reporting';
	private const SLUG                = 'as-ms-reporting';
	private const ASSET_NAME          = 'as-ms-reporting.zip';
	private const CHECK_QUERY_KEY     = 'asms_check_updates';
	private const RELEASE_TRANSIENT   = 'asms_github_latest_release';
	private const ERROR_TRANSIENT     = 'asms_github_latest_release_error';
	private const BACKOFF_TRANSIENT   = 'asms_github_release_backoff';
	private const MANIFEST_URL        = 'https://raw.githubusercontent.com/cchatterton/as-ms-reporting/main/update.json';
	private const REPOSITORY_URL      = 'https://github.com/cchatterton/as-ms-reporting';
	...
	public function add_update_data( $transient ) {
		...
		$plugin_file        = plugin_basename( ASMS_PLUGIN_FILE );
		...
		if ( ! is_array( $release ) || ! version_compare( $release['version'], ASMS_VERSION, '>' ) ) {
			return $transient;
		}
	...
	public function plugin_information( $result, $action, $args ) {
		...
		return (object) array(
			'name'          => 'AS Managed Services Reporting',
			'slug'          => self::SLUG,
			...
			'requires_php'  => self::PHP_VERSION,
			'sections'      => array(
				'description' => 'Managed-services account reporting, access controls, data imports, and AI-assisted summaries.',
	...
	public function plugin_row_meta( $links, $file ) {
		if ( plugin_basename( ASMS_PLUGIN_FILE ) !== $file || ! current_user_can( 'update_plugins' ) ) {
			return $links;
		}
	...
	public function handle_manual_check() {
		...
		if ( ! current_user_can( 'update_plugins' ) ) {
			wp_die( esc_html__( 'You are not allowed to check plugin updates.', 'as-ms-reporting' ) );
		}
		...
		$plugins_url = is_network_admin() ? network_admin_url( 'plugins.php' ) : admin_url( 'plugins.php' );
		wp_safe_redirect( add_query_arg( 'asms_update_check', $result, $plugins_url ) );
		exit;
	}
	...
	public function manual_check_notice() {
		if ( empty( $_GET['asms_update_check'] ) || ! current_user_can( 'update_plugins' ) ) {
			return;
		}

		$result = sanitize_key( wp_unslash( $_GET['asms_update_check'] ) );
		$notices = array(
			'available' => array( 'warning', 'An AS Managed Services Reporting update is available below.' ),
			'current'   => array( 'success', 'AS Managed Services Reporting is current.' ),
	...
	public function clear_cache_after_update( $upgrader, $options ) {
		...
		if ( in_array( plugin_basename( ASMS_PLUGIN_FILE ), $plugins, true ) ) {
			$this->clear_release_state();
		}
	}
	...
	private function get_latest_release( $force ) {
		...
		$duration = version_compare( $release['version'], ASMS_VERSION, '>' ) ? 6 * HOUR_IN_SECONDS : 5 * MINUTE_IN_SECONDS;
	...
	private function get_manifest_release() {
		$response = wp_remote_get(
			...
			array(
				'timeout' => 10,
				'headers' => array( 'User-Agent' => 'AS-Managed-Services-Reporting/' . ASMS_VERSION ),
			)
		);
		...
		$response = wp_remote_get(
			...
			array(
				...
				'headers' => array(
					...
					'User-Agent' => 'AS-Managed-Services-Reporting/' . ASMS_VERSION,
				),
			)
		);
